...
if const.allow_get_user_number_a_segment <= have_seen_guest_number then
	http_response.response_segment_allow_guest_number_uplimit()
end
...
if nil == dealed_guests[index] then
	http_response.response_no_guest()
end
...
if count < const.return_number_when_getting_user_ids_by_sex then 
	total_available = count
else
	total_available = const.return_number_when_getting_user_ids_by_sex
end

if total_available - have_seen_guest_number <= 0 then 
	http_response.response_no_more_guest()
else
	return_number = total_available - have_seen_guest_number
end
if return_number > total_available - today_have_seen_number then
	return_number = total_available - today_have_seen_number
end


repeat 
	loop_time = loop_time - 1
	index = random.number(1, count)
	if helper.in_array(dealed_guests[index]["owner_id"], log.target_ids) then
		table.remove(dealed_guests, index)
		count = count - 1;
	else
		return_number = return_number - 1
		count = count - 1;
		local target_id = dealed_guests[index]["owner_id"]

		local light_log_collection = db:get_col("light_log")
		local have_light_number = light_log_collection:count({
				["available"] = true,
				["user_id"] = args.user_id,
				["target_id"] = target_id
			}
		...
		local one_user_id = {
			...
			["have_light_number"] = have_light_number
		}
		table.insert(user_ids, one_user_id)
		table.remove(dealed_guests, index)
	end
until return_number == 0 or loop_time == 0 or count == 0
