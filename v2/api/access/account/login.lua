...
local check_item = form_check_config.login
local result, err = form_validation.check(args, check_item)
if false == result then
	output.invalid_request(err, err)
end

if nil == args[args.login_type] then 
	err = zh_lang.type_data_not_match
	output.invalid_request(err, err)
end

local s_account = loadfile(ngx.var.root .. "/v2/service/account.lua")():new()
local result = s_account[args.login_type .. "_login_check"](s_account, args.phone_number, args[args.login_type])
if false == result then
	local msg = zh_lang.login_failed
	output.invalid_request(msg, msg)
elseif nil == result then
	local msg = ""
	output.server_error(msg, msg)
end